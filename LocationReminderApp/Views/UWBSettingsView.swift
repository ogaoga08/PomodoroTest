...
class UWBManager: NSObject, ObservableObject, CLLocationManagerDelegate {
    ...
    // スレッドセーフティ用のシリアルキュー
    private let niSessionQueue = DispatchQueue(label: "com.locationreminder.app.uwb.niSessionQueue")
    ...
    // 指数バックオフ用のプロパティ
    // BGTaskSchedulerは実行が不確実なため、補助的な役割に留める
    private var currentBGTaskInterval: TimeInterval = 900 // 初期値: 900秒（15分）
    private let minBGTaskInterval: TimeInterval = 900 // 最小: 900秒（15分）
    ...
    // 距離計測開始確認用
    private var distanceMeasurementCheckTasks: [Int: DispatchWorkItem] = [:]  // デバイス毎の確認タスク
    private let distanceMeasurementTimeout: TimeInterval = 5.0  // 距離計測開始のタイムアウト（秒）- バックグラウンド対応で短縮
    private var distanceMeasurementRetryCount: [Int: Int] = [:]  // デバイス毎のリトライ回数
    private let maxDistanceMeasurementRetries = 5  // 最大リトライ回数（5秒×5回=25秒）
    ...
    // MARK: - スレッドセーフなNISessionアクセスヘルパー
    
    /// スレッドセーフにNISessionを取得
    private func getNISession(for deviceID: Int) -> NISession? {
        return niSessionQueue.sync {
            return niSessions[deviceID]
        }
    }
    
    /// スレッドセーフにNISessionを設定
    private func setNISession(_ session: NISession?, for deviceID: Int) {
        niSessionQueue.sync {
            niSessions[deviceID] = session
        }
    }
    
    /// スレッドセーフにNISessionを削除
    private func removeNISession(for deviceID: Int) {
        niSessionQueue.sync {
            niSessions[deviceID]?.invalidate()
            niSessions.removeValue(forKey: deviceID)
        }
    }
    
    /// スレッドセーフにNISessionの存在を確認
    ...
    func disconnectFromDevice(_ device: UWBDevice) {
        ...
        // NISessionを無効化（スレッドセーフ）
        removeNISession(for: device.uniqueID)
    ...
    private func requestNearbyInteractionPermissionCorrectly() {
        ...
        // ⚠️ 重要: NISession()は必ずメインスレッドで作成する（Qorvoサンプルコードの仕様）
        guard Thread.isMainThread else {
            DispatchQueue.main.async {
                self.requestNearbyInteractionPermissionCorrectly()
            }
            return
        }
        
        // 許可テスト用セッションを作成（メインスレッド）
    ...
    private func handleAccessoryConfigurationData(_ configData: Data, device: UWBDevice) {
        do {
            ...
            // ⚠️ 重要: NISession()は必ずメインスレッドで作成する（Qorvoサンプルコードの仕様）
            DispatchQueue.main.async {
                // 既存のNISessionがある場合は無効化（スレッドセーフ）
                if let existingSession = self.getNISession(for: device.uniqueID) {
                    existingSession.invalidate()
                    self.logger.info("既存NISessionを無効化: \(device.name)")
                }
                
                // NISessionを開始（ここで許可ダイアログが表示される）
                let session = NISession()
                session.delegate = self
                self.setNISession(session, for: device.uniqueID)
                ...
                self.logger.info("📱 新しいNISession開始: \(device.name)")
            }
    ...
    private func handleUWBDidStart(device: UWBDevice) {
        ...
        // 距離計測開始確認タスクをキャンセル（成功したため）
        distanceMeasurementCheckTasks[deviceID]?.cancel()
        distanceMeasurementCheckTasks.removeValue(forKey: deviceID)
        distanceMeasurementRetryCount.removeValue(forKey: deviceID)
                ...
                if peripheral.state == .connected {
                    ...
                    DispatchQueue.global(qos: .userInitiated).asyncAfter(deadline: .now() + 2.0) {
                        ...
                        if let existingDevice = self.discoveredDevices.first(where: { $0.peripheral.identifier == peripheral.identifier }) {
                            ...
                            let hasNISession = self.hasNISession(for: deviceID)
    ...
    private func startRepairProcess(for device: UWBDevice, error: Error) {
        ...
        // 即座に再ペアリング試行（固定遅延）
        // ⚠️ 修正: attemptNISessionRepairは内部でメインスレッドに切り替えるため、
        // ここではバックグラウンドスレッドからの呼び出しでも安全
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
    ...
    private func attemptNISessionRepair(for device: UWBDevice) {
        ...
        // スレッドセーフのため、メインスレッドで辞書読み取り（+1はしない）
        let currentAttempt: Int
        if Thread.isMainThread {
            currentAttempt = repairAttempts[deviceID, default: 0]
        } else {
            currentAttempt = DispatchQueue.main.sync {
                return repairAttempts[deviceID, default: 0]
            }
        }
        
        logger.info("🔄 再ペアリング試行 #\(currentAttempt + 1): \(device.name)")
        ...
        // ⚠️ 重要: Qorvoサンプルコードに従い、NISession()は必ずメインスレッドで作成する
        DispatchQueue.main.async {
            // 既存のNISessionをクリーンアップ（スレッドセーフ）
            if let existingSession = self.getNISession(for: deviceID) {
                self.logger.info("🗑️ 既存NISession無効化")
                existingSession.invalidate()
            }
            
            // NISessionの再作成を試行（メインスレッドで実行）
            self.logger.info("🆕 新しいNISession作成（メインスレッド）")
            let newSession = NISession()
            newSession.delegate = self
            
            // スレッドセーフに辞書へ保存
            self.setNISession(newSession, for: deviceID)
            
            // バックグラウンドモードの場合は特別な設定を適用
            if self.isBackgroundMode {
                self.setupSessionForBackgroundMode(newSession)
            }
            
            // ⚠️ 重要: Qorvoサンプルコードに従い、既存の設定を再利用する
            // 再ペアリング時は、既にペアリング済みなので設定データは保存されている
            // session.run()を実行すると、didGenerateShareableConfigurationDataが呼ばれ、
            // configureAndStartメッセージがデバイスに送信される
            self.logger.info("▶️ NISession.run()実行（既存設定を使用）")
            newSession.run(configuration)
            
            self.logger.info("🔄 NISession再開始完了: \(device.name)")
            self.logger.info("   👉 次のステップ: session(_:didGenerateShareableConfigurationData:)の呼び出し待ち")
            
            // 試行回数をインクリメント（メインスレッドで実行中）
            self.repairAttempts[deviceID] = currentAttempt + 1
    ...
    // 🔧 修正: 再ペアリング成功後は単にinitializeメッセージを送信するだけ
    private func ensureDistanceMeasurementStarted(for device: UWBDevice) {
        ...
        // Qorvoサンプルコードに従い、initializeメッセージを送信するだけ
        // 距離データはデバイスに近づいた時にsession(_:didUpdate:)で自動的に取得される
        let initMessage = Data([MessageId.initialize.rawValue])
        sendDataToDevice(initMessage, device: device)
        ...
        logger.info("✅ 距離計測準備完了")
        logger.info("   👉 bluetooth-central + nearby-interactionモードにより")
        logger.info("   👉 デバイスに近づいた時に自動的にsession(_:didUpdate:)が呼ばれます")
    }
    
    // 🗑️ 削除: 複雑な検証ロジックは不要（bluetooth-centralモードで自動動作）
    // 距離計測が開始されているか確認し、必要に応じて再試行（削除予定）
    private func verifyDistanceMeasurementStarted(for device: UWBDevice) {
        let deviceID = device.uniqueID
        
        // タスクがキャンセルされていたら何もしない
        guard distanceMeasurementCheckTasks[deviceID] != nil else {
            return
        }
        ...
        let currentRetry = distanceMeasurementRetryCount[deviceID, default: 0]
        
        logger.info("🔍 距離計測開始確認 (試行 \(currentRetry + 1)/\(self.maxDistanceMeasurementRetries + 1)): \(device.name)")
        logger.info("   - デバイスステータス: \(device.status.rawValue)")
        logger.info("   - 距離データ: \(device.distance != nil ? "有" : "無")")
        logger.info("   - Bluetooth接続: \(device.peripheral.state == .connected ? "接続" : "未接続")")
        logger.info("   - NISession: \(self.hasNISession(for: deviceID) ? "有" : "無")")
        
        // 距離計測が開始されている（rangingステータスまたは距離データあり）場合は成功
        if device.status == .ranging || device.distance != nil {
            logger.info("✅ 距離計測開始成功: \(device.name)")
            distanceMeasurementRetryCount.removeValue(forKey: deviceID)
            return
        }
        
        // まだconnectedまたはpairedの状態で、距離データがない場合は再試行
        if (device.status == .connected || device.status == .paired) && 
           device.peripheral.state == .connected &&
           self.hasNISession(for: deviceID) {
            
            // 最大リトライ回数に達していない場合は再試行
            if currentRetry < self.maxDistanceMeasurementRetries {
                distanceMeasurementRetryCount[deviceID] = currentRetry + 1
                
                logger.warning("⚠️ 距離計測が開始されていません - 再試行 \(currentRetry + 2)/\(self.maxDistanceMeasurementRetries + 1): \(device.name)")
                
                // バックグラウンドの場合は通知を送信（最初と最後のみ）
                if isBackgroundMode && (currentRetry == 0 || currentRetry == self.maxDistanceMeasurementRetries - 1) {
                    sendUWBPairingDebugNotification(
                        title: "⚠️ 距離計測未開始",
                        message: "initializeメッセージを再送信します (\(currentRetry + 2)/\(self.maxDistanceMeasurementRetries + 1))",
                        deviceName: device.name
                    )
                }
                
                // 再度initializeメッセージを送信
                let initMessage = Data([MessageId.initialize.rawValue])
                sendDataToDevice(initMessage, device: device)
                logger.info("📤 initializeメッセージ再送信: \(device.name)")
                
                // bluetooth-central + nearby-interactionモードにより
                // デバイスに近づいた時に自動的にsession(_:didUpdate:)が呼ばれるため、
                // 追加の確認は不要
            } else {
                // 最大リトライ回数に達した
                logger.error("❌ 距離計測開始失敗（最大リトライ回数到達）: \(device.name)")
                distanceMeasurementRetryCount.removeValue(forKey: deviceID)
                
                if isBackgroundMode {
                    sendUWBPairingDebugNotification(
                        title: "❌ 距離計測開始失敗",
                        message: "バックグラウンドでの開始に失敗しました。アプリを開くと再試行されます。",
                        deviceName: device.name
                    )
                }
            }
        } else {
            logger.error("❌ 距離計測開始条件不足: \(device.name)")
            logger.error("   - デバイスステータス: \(device.status.rawValue)")
            logger.error("   - Bluetooth状態: \(device.peripheral.state.rawValue)")
            logger.error("   - NISession存在: \(self.hasNISession(for: deviceID))")
            distanceMeasurementRetryCount.removeValue(forKey: deviceID)
        }
    }
    ...
    private func verifyRepairSuccess(for device: UWBDevice) {
        ...
        let sessionExists = self.hasNISession(for: deviceID)
        let bluetoothConnected = device.peripheral.state == .connected
        
        logger.info("🔍 再ペアリング検証: \(device.name)")
        logger.info("   - NISession存在: \(sessionExists)")
        logger.info("   - Bluetooth接続: \(bluetoothConnected)")
        logger.info("   - デバイスステータス: \(device.status.rawValue)")
        ...
        if sessionExists && bluetoothConnected {
            ...
            // 検証失敗
            logger.warning("⚠️ 再ペアリング検証失敗: \(device.name)")
            
            if isBackgroundMode {
                // バックグラウンドモードでは、次のハートビートで再試行される
                // repairingDeviceIDはそのまま維持（processBackgroundRepairで処理される）
                logger.info("⏳ バックグラウンド: 次のハートビートで再試行")
            } else {
                // フォアグラウンドでは次の試行をスケジュール
                logger.info("⏳ フォアグラウンド: 次の試行をスケジュール")
    ...
    private func scheduleNextRepairAttempt(for device: UWBDevice, delay: TimeInterval? = nil) {
        ...
        // スレッドセーフのため、メインスレッドで辞書読み取り（インクリメントはしない）
        let currentAttempts: Int
        if Thread.isMainThread {
            currentAttempts = repairAttempts[deviceID, default: 0]
        } else {
        ...
        let calculatedDelay = baseRepairInterval * pow(2.0, Double(currentAttempts)) * backoffMultiplier
        ...
        logger.info("⏱️ 次の再ペアリング試行を\(Int(waitTime))秒後にスケジュール")
        
        // バックグラウンドでのタイマー実行を改善
        if self.isBackgroundMode {
            // バックグラウンドタスクとして実行（メインスレッドで実行）
            DispatchQueue.main.asyncAfter(deadline: .now() + waitTime) {
                self.attemptNISessionRepair(for: device)
            }
        } else {
            // フォアグラウンドでは通常のタイマーを使用（メインスレッドで実行）
    ...
    private func beginBackgroundTask() {
        // ⚠️ 重要: bluetooth-centralとnearby-interactionモードが有効な場合、
        // UIBackgroundTaskは不要（自動的にバックグラウンド実行が継続される）
        logger.info("ℹ️ bluetooth-central + nearby-interactionモードによりバックグラウンド実行継続")
        
        // 念のため、短期的なタスクのみ開始（オプション）
        endBackgroundTask()
        backgroundTaskIdentifier = UIApplication.shared.beginBackgroundTask(withName: "UWB Initial Transition") {
            self.logger.info("⏱️ 初期移行タスク終了（bluetooth-centralモードに移行）")
            self.endBackgroundTask()
        }
        
        if backgroundTaskIdentifier != .invalid {
            logger.info("📱 初期移行タスク開始: \(self.backgroundTaskIdentifier.rawValue)")
            
            // 30秒後に自動終了（bluetooth-centralモードに完全移行）
    ...
    private func transitionToBackgroundMode() {
        ...
        logger.info("ℹ️ bluetooth-central + nearby-interactionモードにより継続実行")
        ...
        if let connectedDevice = discoveredDevices.first(where: { 
            ...
            let deviceID = connectedDevice.uniqueID
            let hasNISession = self.hasNISession(for: deviceID)
            let hasDistance = connectedDevice.distance != nil
            
            logger.info("🔌 接続済みデバイス: \(connectedDevice.name)")
            logger.info("   - NISession: \(hasNISession ? "有" : "無")")
            logger.info("   - 距離データ: \(hasDistance ? "有" : "無")")
            
            // NISessionがあれば、bluetooth-centralとnearby-interactionモードにより
            // デバイスに近づいた時に自動的にsession(_:didUpdate:)が呼ばれる
            if hasNISession {
                logger.info("✅ NISession継続中 - デバイスに近づけば自動的に距離データ更新")
            }
        }
        
        // バックグラウンド処理の開始（bluetooth-centralモードで継続）
    ...
    private func transferBackgroundRepairToForeground() {
        ...
        // ⚠️ 修正: メインスレッドで実行
        DispatchQueue.main.asyncAfter(deadline: .now() + shortDelay) {
            self.attemptNISessionRepair(for: device)
        }
    ...
    private func checkAndRestoreNISessionsOnForeground() {
        guard let connectedDevice = discoveredDevices.first(where: { 
            ...
            logger.info("フォアグラウンド復帰: 接続済みデバイスなし")
            return
        }
        ...
        logger.info("📱 フォアグラウンド復帰時チェック: \(connectedDevice.name)")
        logger.info("   - NISession: \(hasNISession ? "有" : "無")")
        logger.info("   - 設定: \(hasConfiguration ? "有" : "無")")
        logger.info("   - Bluetooth: \(bluetoothConnected ? "接続" : "未接続")")
        logger.info("   - デバイスステータス: \(connectedDevice.status.rawValue)")
        logger.info("   - 距離データ: \(connectedDevice.distance != nil ? "有" : "無")")
        
        // Bluetooth未接続の場合はスキップ
        guard bluetoothConnected else {
            logger.warning("⚠️ Bluetooth未接続 - 復旧スキップ")
            return
        }
        
        // NISessionがない場合は再作成
        if !hasNISession && hasConfiguration {
            logger.info("🔄 NISession不足 - 再作成: \(connectedDevice.name)")
        ...
        // NISessionはあるが距離計測が開始されていない場合
        if hasNISession && hasConfiguration && connectedDevice.distance == nil {
            logger.info("🔄 距離計測未開始 - Qorvoパターンで再初期化: \(connectedDevice.name)")
            let delay: TimeInterval = 2.0
            DispatchQueue.main.asyncAfter(deadline: .now() + delay) {
                // Qorvoパターン: initializeメッセージを送信
                let initMessage = Data([MessageId.initialize.rawValue])
                self.sendDataToDevice(initMessage, device: connectedDevice)
                self.logger.info("📤 フォアグラウンド復帰: initializeメッセージ送信")
            }
            return
        }
        
        logger.info("✅ フォアグラウンド復帰: 修復不要")
    }
    ...
    private func startBackgroundHeartbeat() {
        ...
        logger.info("📱 bluetooth-central + nearby-interactionモード開始")
        logger.info("ℹ️ NISessionのデリゲートメソッドがバックグラウンドでも自動実行されます")
        
        // bluetooth-centralとnearby-interactionモードが有効な場合：
        // - Bluetooth接続が自動的に維持される
        // - NISessionのデリゲートメソッド（didUpdate, sessionSuspensionEnded等）がバックグラウンドでも呼ばれる
        // - デバイスに近づいた時、session(_:didUpdate:)が自動的に呼ばれる
        
        // 接続維持のための軽量なハートビート（bluetooth-centralモードで継続）
        scheduleBluetoothHeartbeat()
    }
    
    private func scheduleBluetoothHeartbeat() {
        guard isBackgroundMode else {
            logger.info("フォアグラウンドに復帰したためハートビート停止")
            return
        }
        
        // bluetooth-centralモードでは、Bluetooth通信イベント（データ送受信）により
        // バックグラウンド実行が継続される
        // 60秒ごとに軽量なメッセージを送信して接続を維持
        DispatchQueue.global(qos: .background).asyncAfter(deadline: .now() + 60.0) { [weak self] in
            guard let self = self else { return }
            
            if self.isBackgroundMode {
                self.performBluetoothHeartbeat()
                // 次のハートビートをスケジュール
                self.scheduleBluetoothHeartbeat()
            }
        }
    }
    ...
    private func performBluetoothHeartbeat() {
        guard isBackgroundMode else {
            return
        }
        
        let elapsed = backgroundHeartbeatStartTime?.timeIntervalSinceNow ?? 0
        logger.info("🕒 Bluetoothハートビート (\(String(format: "%.0f", abs(elapsed)))秒経過)")
        ...
        guard connectedDevice.peripheral.state == .connected else {
            logger.warning("⚠️ Bluetooth未接続")
            return
        }
        
        // bluetooth-centralモードでバックグラウンド実行を継続するために
        // 軽量なBluetoothメッセージを送信（接続維持）
        ...
        logger.info("📡 BG継続: NISession=\(hasNISession ? "有" : "無") | 距離=\(hasDistance ? "有" : "無")")
        
        // NISessionがない場合は再作成（稀なケース）
        if !hasNISession && accessoryConfigurations[deviceID] != nil {
            logger.warning("⚠️ NISession喪失検出 - 再作成")
            attemptNISessionRepair(for: connectedDevice)
        }
    }
    ...
    private func checkAndRepairNISessionIfNeeded(for device: UWBDevice) {
        ...
        // デバイスがconnected以上のステータスで、設定データがあるのにNISessionがない場合は修復対象
        let shouldHaveNISession = (device.status == .ranging || device.status == .paired || device.status == .connected) && hasConfiguration
        
        // NISessionが必要なのに存在しない場合のみ修復対象とする
        let shouldRepair = !hasNISession && shouldHaveNISession
        
        // まずNISessionが復旧したかチェック
        if hasNISession && repairingDeviceID == deviceID {
            // NISessionが復旧したので再ペアリング対象から外す
            logger.info("   ✅ NISession復旧確認: \(device.name)")
            repairingDeviceID = nil
            // 試行回数もリセット
            DispatchQueue.main.async {
                self.repairAttempts.removeValue(forKey: deviceID)
            }
            return
        }
        ...
        // 試行回数をチェック
        let currentAttempts = repairAttempts[deviceID, default: 0]
        if currentAttempts >= maxRepairAttempts {
            logger.info("   ⚠️ 最大試行回数到達済み: \(device.name) (\(currentAttempts)/\(self.maxRepairAttempts))")
            // 最大試行回数に達したら、再ペアリング対象から外す
            if repairingDeviceID == deviceID {
                repairingDeviceID = nil
            }
            return
        ...
        // 再ペアリング対象に設定（まだ設定されていない場合のみ）
        if repairingDeviceID == nil {
            repairingDeviceID = deviceID
            logger.info("   ⚠️ NISession不足検出: \(device.name) (ステータス: \(device.status.rawValue), NISession: \(hasNISession), 設定: \(hasConfiguration))")
        } else if repairingDeviceID != deviceID {
            // 他のデバイスが既に対象になっている
            logger.info("   ℹ️ 他のデバイスが再ペアリング中のためスキップ")
        }
    }
    ...
    private func processBackgroundRepair() {
        ...
        if timeSinceLastAttempt >= minIntervalBetweenAttempts {
            ...
            // 最大試行回数をチェック
            if currentAttempts >= maxRepairAttempts {
                logger.warning("   ⚠️ 再ペアリング最大試行回数に到達: \(device.name) (\(currentAttempts)回)")
                repairingDeviceID = nil
                // 試行回数もリセット
                DispatchQueue.main.async {
                    self.repairAttempts.removeValue(forKey: deviceID)
            ...
            logger.info("   🔄 再ペアリング実行: \(device.name) (試行 \(currentAttempts + 1)/\(self.maxRepairAttempts))")
    ...
    private func ensureBackgroundUWBRecovery() {
        ...
        logger.info("🔧 UWB復旧処理開始（BGTaskScheduler）")
        ...
        if let device = discoveredDevices.first(where: { $0.status == .connected || $0.status == .paired || $0.status == .ranging }) {
            ...
            if bluetoothConnected {
                // NISessionがない場合は再作成
                if !hasNISession && hasConfiguration {
                    logger.info("🔄 NISession不足 - 再作成実行")
                    attemptNISessionRepair(for: device)
                }
                // NISessionはあるが距離データがない場合は再初期化
                else if hasNISession && hasConfiguration && !hasDistance {
                    logger.info("🔄 距離データ不足 - Qorvoパターンで再初期化")
                    let initMessage = Data([MessageId.initialize.rawValue])
                    sendDataToDevice(initMessage, device: device)
                }
            } else {
                logger.warning("⚠️ Bluetooth未接続 - 再接続が必要")
            }
        } else {
            logger.info("⚠️ 接続済みデバイスなし")
        }
    ...
    func optimizeForBackgroundMode() {
        ...
        // アクティブでないセッションの停止（スレッドセーフ）
        var sessionsToRemove: [Int] = []
        
        niSessionQueue.sync {
            for (deviceID, _) in niSessions {
                if let device = findDevice(uniqueID: deviceID),
                   device.status != .ranging {
                    sessionsToRemove.append(deviceID)
                }
            }
        }
...
extension UWBManager: CBCentralManagerDelegate {
    ...
    func centralManager(_ central: CBCentralManager, didDisconnectPeripheral peripheral: CBPeripheral, error: Error?) {
        ...
        if let device = findDevice(peripheral: peripheral) {
            ...
            if self.hasNISession(for: device.uniqueID) {
                logger.info("デバイス切断により再ペアリングを開始: \(device.name)")
                
                // NISessionを明示的に無効化（スレッドセーフ）
                removeNISession(for: device.uniqueID)
...
extension UWBManager: CBPeripheralDelegate {
    ...
    func peripheral(_ peripheral: CBPeripheral, didWriteValueFor characteristic: CBCharacteristic, error: Error?) {
        guard let device = findDevice(peripheral: peripheral) else { return }
        
        if let error = error {
            logger.error("❌ データ送信エラー (\(device.name)): \(error.localizedDescription)")
            
            // バックグラウンドの場合はエラー通知を送信
            if isBackgroundMode {
                sendUWBPairingDebugNotification(
                    title: "❌ 通信エラー",
            ...
            logger.info("✅ データ送信成功 (\(device.name)): characteristic \(characteristic.uuid)")
        }
    }
}
...
extension UWBManager: NISessionDelegate {
    ...
    func sessionWasSuspended(_ session: NISession) {
        ...
        if let device = targetDevice {
            ...
            // Qorvoサンプルコードと同じパターン: stopメッセージを送信
            if device.peripheral.state == .connected {
                let stopMessage = Data([MessageId.stop.rawValue])
                sendDataToDevice(stopMessage, device: device)
                logger.info("📤 一時停止: stopメッセージ送信")
            }
    ...
    func sessionSuspensionEnded(_ session: NISession) {
        ...
        if let device = targetDevice {
            logger.info("🔄 セッション再開 - Qorvoパターンで再初期化: \(device.name)")
            
            // Qorvoサンプルコードと同じパターン: initializeメッセージを送信
            if device.peripheral.state == .connected {
                DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
                    let initMessage = Data([MessageId.initialize.rawValue])
                    self.sendDataToDevice(initMessage, device: device)
                    self.logger.info("📤 サスペンド終了後のinitializeメッセージ送信: \(device.name)")
                }
            } else {
    ...
    func session(_ session: NISession, didGenerateShareableConfigurationData shareableConfigurationData: Data, for object: NINearbyObject) {
        ...
        // ⚠️ 重要: このメソッドは以下の場合に呼ばれる
        // 1. 初回ペアリング時（session.run()実行後）
        // 2. 再ペアリング時（session.run()実行後）
        // 3. サスペンド復帰時
        // Qorvoサンプルコードでは、このメソッドが呼ばれるたびにconfigureAndStartを送信している
        
        // デバッグ通知: ステータスがconnected（初回）の場合のみ送信
        if device.status == .connected && !self.isRepairing {
            sendUWBPairingDebugNotification(
                title: "🔄 UWBペアリング開始",
            ...
            logger.info("ℹ️ 再ペアリングまたはサスペンド復帰のため、通知をスキップ")
        }
        
        // 設定データを送信（Qorvoサンプルコードと同じ）
    ...
    func session(_ session: NISession, didUpdate nearbyObjects: [NINearbyObject]) {
        ...
        // 初回の距離データ取得時のログ（または距離データ復旧時）
        let isFirstDistance = device.distance == nil
        if isFirstDistance {
            if isBackgroundMode {
                logger.info("🎯✨ バックグラウンドで距離データ復旧: \(device.name) - \(String(format: "%.2f", distance))m")
                logger.info("✅ bluetooth-central + nearby-interactionモードにより自動復旧成功")
                
                sendUWBPairingDebugNotification(
                    title: "🎯 距離計測自動復旧",
                    message: "バックグラウンドで距離: \(String(format: "%.2f", distance))m",
                    deviceName: device.name
                )
            } else {
    ...
    func session(_ session: NISession, didRemove nearbyObjects: [NINearbyObject], reason: NINearbyObject.RemovalReason) {
        ...
        switch reason {
        case .timeout:
            logger.info("⏱️ NISessionタイムアウト - Qorvoパターンで再初期化: \(device.name)")
            // Qorvoサンプルコードと同じパターン: stop → initialize
            retryDistanceMeasurement(for: device)
            
        case .peerEnded:
            logger.info("🔌 ピアが終了（距離が離れた可能性） - Qorvoパターンで再初期化: \(device.name)")
            // 距離が離れた場合も同じパターンで再試行
            retryDistanceMeasurement(for: device)
            
        @unknown default:
            logger.info("❓ 不明な削除理由 - Qorvoパターンで再初期化: \(device.name)")
            retryDistanceMeasurement(for: device)
        }
    }
    
    // Qorvoサンプルコードのパターン: stop → initialize
    private func retryDistanceMeasurement(for device: UWBDevice) {
        guard device.peripheral.state == .connected else {
            logger.warning("⚠️ Bluetooth未接続 - 再初期化スキップ")
            return
        }
        
        logger.info("🔄 距離計測再初期化: \(device.name) (バックグラウンド: \(self.isBackgroundMode))")
        
        // 🔑 重要: Qorvoサンプルコードと同じパターン
        // stop → initializeの順で送信（間隔を短くしてbluetooth-centralモードで確実に実行）
        
        // 1. stopメッセージを送信
        let stopMessage = Data([MessageId.stop.rawValue])
        sendDataToDevice(stopMessage, device: device)
        logger.info("📤 stopメッセージ送信")
        
        // 2. 0.5秒後にinitializeメッセージを送信
        // bluetooth-centralモードでは、Bluetooth通信により実行時間が延長される
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
            let initMessage = Data([MessageId.initialize.rawValue])
            self.sendDataToDevice(initMessage, device: device)
            self.logger.info("📤 initializeメッセージ送信: \(device.name)")
            
            // バックグラウンドの場合は通知
            if self.isBackgroundMode {
                self.sendUWBPairingDebugNotification(
                    title: "🔄 距離計測再初期化",
                    message: "デバイスに近づいたら自動的に距離データが更新されます",
                    deviceName: device.name
                )
            }
        }
    ...
    func session(_ session: NISession, didInvalidateWith error: Error) {
        ...
        if let deviceID = targetDeviceID {
            ...
            // デバイス状態をリセット
            if let device = findDevice(uniqueID: deviceID) {
                logger.info("⚠️ NISession無効化: \(device.name) - エラー: \(error.localizedDescription)")
                ...
                // Qorvoサンプルコードと同じパターン: 新しいNISessionを作成してinitializeを送信
                // Bluetooth接続が維持されている場合のみ
                if device.peripheral.state == .connected {
                    logger.info("🔄 Qorvoパターンでセッション再作成: \(device.name)")
                    
                    DispatchQueue.main.asyncAfter(deadline: .now() + 2.0) {
                        self.attemptNISessionRepair(for: device)
                    }
                } else {
                    logger.warning("⚠️ Bluetooth未接続 - セッション再作成スキップ")
                }
            }
        }
    }
...
struct UWBSettingsView: View {
    ...
    // 計算プロパティで複雑な式を分割してコンパイル時間を短縮
    private var secureBubbleIconName: String {
        uwbManager.isInSecureBubble ? "checkmark.circle.fill" : "xmark.circle.fill"
    }
    
    private var secureBubbleIconColor: Color {
        uwbManager.isInSecureBubble ? .red : .green
    }
    
    private var secureBubbleStatusText: String {
        uwbManager.isInSecureBubble ? "内部" : "外部"
    }
    
    private var backgroundModeIconName: String {
        uwbManager.isBackgroundMode ? "moon.circle.fill" : "sun.max.circle.fill"
    }
    
    private var backgroundModeIconColor: Color {
        uwbManager.isBackgroundMode ? .blue : .orange
    }
    
    private var backgroundModeText: String {
        uwbManager.isBackgroundMode ? "バックグラウンド" : "フォアグラウンド"
    }
    ...
    var body: some View {
        ...
        .alert("Nearby Interaction許可が必要", isPresented: $showingNIPermissionAlert) {
            Button("OK", role: .cancel) { }
        } message: {
            Text("UWB通信を使用するには、Nearby Interactionの許可が必要です。")
        }
    }
    ...
    // MARK: - UWB Status Section
    @ViewBuilder
    private var uwbStatusSection: some View {
        if uwbManager.isUWBActive {
            VStack(spacing: 8) {
                HStack {
                    Image(systemName: secureBubbleIconName)
                        .foregroundColor(secureBubbleIconColor)
                    Text("Secure Bubble: \(secureBubbleStatusText)")
                        .font(.caption)
                        .foregroundColor(.secondary)
                    Spacer()
                }
                ...
                // バックグラウンド状態表示
                HStack {
                    Image(systemName: backgroundModeIconName)
                        .foregroundColor(backgroundModeIconColor)
                    Text("動作モード: \(backgroundModeText)")
                        .font(.caption)
                        .foregroundColor(.secondary)
                    Spacer()
                }
    ...
    // MARK: - Disconnect Button Section
    @ViewBuilder
    private var disconnectButtonSection: some View {
    ...
    @ViewBuilder
    private var diagnosticInfoView: some View {
        if let connectedDevice = uwbManager.discoveredDevices.first(where: {
            $0.status == .connected || $0.status == .paired || $0.status == .ranging
        }) {
            DiagnosticInfoContentView(
...
// MARK: - Diagnostic Info Content View
private struct DiagnosticInfoContentView: View {
    let device: UWBDevice
    let uwbManager: UWBManager
    
    private var deviceID: Int { device.uniqueID }
    private var hasNISession: Bool { uwbManager.hasNISession(for: deviceID) }
    private var hasConfiguration: Bool { uwbManager.hasConfiguration(for: deviceID) }
    
    private var niSessionStatusText: String { hasNISession ? "作成済み" : "なし" }
    private var niSessionStatusColor: Color { hasNISession ? .green : .red }
    
    private var configStatusText: String { hasConfiguration ? "受信済み" : "なし" }
    private var configStatusColor: Color { hasConfiguration ? .green : .red }
    
    private var bluetoothStateColor: Color {
        device.peripheral.state == .connected ? .green : .orange
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            HStack {
                Text("• NISession:")
                Text(niSessionStatusText)
                    .foregroundColor(niSessionStatusColor)
            }
            .font(.caption)
            ...
            HStack {
                ...
                Text(configStatusText)
                    .foregroundColor(configStatusColor)
            }
            .font(.caption)
...
// MARK: - UWBSettingsView Helper Properties
extension UWBSettingsView {
    private var bluetoothStateColor: Color {
